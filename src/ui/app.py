...
st.markdown("""
<style>
/* Import modern geometric sans-serif font stack */
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');

/* Apply font across the entire layout container */
.stApp {
    font-family: 'Inter', -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif !important;
    background-color: #0B0E14 !important;
}

/* Restyle KPI Blocks into sleek floating glass containers */
...
/* Clean typography weight adjustments for KPIs */
...
/* Modernize all interactive inputs and buttons */
...
/* Style the base chat input container and its internal text area box */
div[data-testid="stChatInput"] textarea {
    border: 1px solid rgba(59, 130, 246, 0.25) !important;
    background-color: #161C2A !important;
    border-radius: 28px !important; /* Premium rounded pill bar style */
    box-shadow: 0 0 20px rgba(59, 130, 246, 0.05) !important;
    transition: all 0.25s ease-in-out !important;
    color: #FFFFFF !important;
}

/* Inject the subtle blue neon glowing state when a user clicks/focuses inside the input */
div[data-testid="stChatInput"] textarea:focus {
    border: 1px solid rgba(59, 130, 246, 0.6) !important;
    box-shadow: 0 0 25px rgba(59, 130, 246, 0.18) !important;
    background-color: #1A2337 !important;
}

/* Restyle the send arrow icon button to match our premium theme aesthetics */
div[data-testid="stChatInput"] button {
    background-color: transparent !important;
    color: #3B82F6 !important; /* Electric blue send action icon color arrow */
    transition: transform 0.2s ease !important;
}
div[data-testid="stChatInput"] button:hover {
    transform: scale(1.1) !important;
}
/* Custom 2-Column Grid for Prompt Cards */
.copilot-grid-layout {
    display: grid;
...
/* Unified Heroicon Vector Badge Wrappers */
.icon-badge {
    display: inline-flex;
    align-items: center;
...
/* Sleek Horizontal Content Pills */
.context-pill {
...
/* Better Conversation Separation */
...
/* Premium Assistant Message Container */
...
with head_col2:
    ...
    with st.popover("⚙️ System Configuration & Settings", use_container_width=True):
        st.markdown("### **Configuration Hub**")
        ...
        with pop_col1:
            ...
            selected_label = st.selectbox("Select AI Provider", list(provider_options.keys()), index=1)
            provider, model_name = provider_options[selected_label]
            
            default_key = os.environ.get("GROQ_API_KEY", "") if provider == "groq" else ""
            api_key = st.text_input("API Key", type="password", value=default_key, help="Overrides local environment variables.")
        ...
        with pop_col2:
            ...
            target_email = st.text_input("Target Manager Email", value="manager@thinkpalm.com")
...
# MAIN LAYOUT
# ==========================================
col_dash, col_chat = st.columns([5.0, 3.0], gap="large")
...
with col_dash:
    ...
    dash_tab1, dash_tab2, dash_tab3, dash_tab4 = st.tabs([
        "📊 Risk Overview", "📉 Top Drivers", "📋 High Risk Roster", "📑 Executive Summary"
    ])
    ...
    with dash_tab1:
        ...
        ''', unsafe_allow_html=True)
        if kpi_col2.button("🔍 Summarize High Risk roster", key="btn_high_risk", use_container_width=True):
            st.session_state.pending_query = f"Summarize the {high_risk} high flight risk employees."
            st.rerun()
...
with col_chat:
    if "agent_graph" not in st.session_state:
        ...
        st.markdown('''
        <style>
        /* Style rule targeting suggestion buttons exclusively */
        ...
        /* Color overrides for specific query buttons */
        ...
        st.markdown('''
        <h3 style="color: #FFFFFF; font-size: 1rem; font-weight: 600; margin-top: 24px; margin-bottom: 12px;">🕒 Recent Sessions</h3>
        ''', unsafe_allow_html=True)
